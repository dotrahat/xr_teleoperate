        ...
        try:
            while self.running:
                ...
                if xr_motion_data_ready:
                    ...
                    #
                    # Motor 0 is thumb FLEXION and motor 1 is thumb ROTATION -- verified on
                    # hardware, and the opposite of what the URDF joint names suggest (see
                    # hand_retargeting.py). The staged ramp below is right either way: rotation
                    # leads and caps at 0.7 so the thumb opposes the fingers first, then flexion
                    # engages over the second half of trigger travel to close the grasp.
                    left_triger_value = (10.0 - left_trigger_value) / 10.0
                    left_q_target[0]  = np.clip((left_triger_value - 0.5) / 0.5, 0.0, 0.98) # thumb flexion
                    left_q_target[1]  = np.clip(left_triger_value / 0.5, 0.0, 0.7) # thumb rotation
                    ...
                    right_q_target[0] = np.clip((right_triger_value - 0.5) / 0.5, 0.0, 0.98) # thumb flexion
                    right_q_target[1] = np.clip(right_triger_value / 0.5, 0.0, 0.7) # thumb rotation
